...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query("""
    SELECT new bg.softuni.cardealer.model.dto.exports.CustomerWithSaleDto(c.name, COUNT(DISTINCT s.car.id), SUM(p.price))
    FROM Customer AS c
    ...
    GROUP BY c.name
""")
    List<CustomerWithSaleDto> getCustomersWithSalesInfo();
}
