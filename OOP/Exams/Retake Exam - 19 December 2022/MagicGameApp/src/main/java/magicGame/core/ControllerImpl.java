...
public class ControllerImpl implements Controller {
    ...
    public String report() {
        Collection<Magician> sortedMagicians = this.magicians.getData().stream().sorted((m1, m2) -> {
            int sort = Integer.compare(m2.getHealth(), m1.getHealth());
            if (sort == 0) {
                sort = m1.getUsername().compareTo(m2.getUsername());
            }
            return sort;
        ...
        for (Magician magician : sortedMagicians) {
            report.append(String.format("%s: %s", magician.getClass().getSimpleName(), magician.getUsername())).append(System.lineSeparator());
            report.append(String.format("Health: %d", magician.getHealth())).append(System.lineSeparator());
            report.append(String.format("Protection: %d", magician.getProtection())).append(System.lineSeparator());
            report.append(String.format("Magic: %s", magician.getMagic().getName())).append(System.lineSeparator());
        }
